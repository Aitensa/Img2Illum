...
class EquFeatBlock(nn.Module):
    def __init__(self, params, norm=None):
        super(EquFeatBlock, self).__init__()
    ...
    def forward(self, x):
        ...
        return output #F.normalize(x_out, p=2, dim=1), attn

class EquOutBlockPointnet(nn.Module):
    def __init__(self, params, norm=None):
        super(EquOutBlockPointnet, self).__init__()
    ...
    def forward(self, x):
        ...
        # batch norm in the last layer?
        # x_out = self.out_norm(x_out)
        return F.normalize(out_feat, p=2, dim=1)#F.normalize(x_out, p=2, dim=1), F.normalize(out_feat, p=2, dim=1)


class EquFeatBlockPointnet(nn.Module):
    def __init__(self, params, norm=None):
        super(EquFeatBlockPointnet, self).__init__()
    ...
    def forward(self, x):
        ...
        # batch norm in the last layer?
        # x_out = self.out_norm(x_out)
        return F.normalize(x_out, p=2, dim=1)#F.normalize(x_out, p=2, dim=1), F.normalize(out_feat, p=2, dim=1)
