...
from datasets.pointar.preprocess.utils import load_pred_rgbd_as_tensor
...
def omit_file(dataset,idx):
    point_ck = os.path.exists(os.path.join(OUTPUT_PATH,dataset, str(idx),'point_cloud.npz'))
    hdr_ck = os.path.exists(os.path.join(OUTPUT_PATH,dataset, str(idx),'shc_hdr.json'))
    ldr_ck = os.path.exists(os.path.join(OUTPUT_PATH,dataset, str(idx),'shc_ldr.json'))
    src_ck = os.path.exists(os.path.join(OUTPUT_PATH,dataset, str(idx),'source.npz'))
    return (point_ck and hdr_ck and ldr_ck and src_ck) 
...
def fetch_data_file(dataset, index, item):
    ...
    # Source image
    img_color, img_depth = load_pred_rgbd_as_tensor(
