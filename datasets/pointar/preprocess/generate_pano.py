...
from datasets.pointar.preprocess.utils import srgb_to_linear
from datasets.pointar.preprocess.utils import load_rgbd_as_tensor_pano
from datasets.pointar.preprocess.utils import get_batched_basis_at

from datasets.pointar.preprocess.utils import cube_uv_to_xyz
from datasets.pointar.preprocess.utils import euler_rotation_xyz
from datasets.pointar.preprocess.utils import cartesian_to_spherical
from datasets.pointar.preprocess.utils import get_euler_rotation_matrix
...
vx = [-np.pi, -2/3*np.pi, -1/3*np.pi, 0, 1/3*np.pi, 2/3*np.pi, -np.pi, -2/3*np.pi, -1/3*np.pi, 0, 1/3*np.pi, 2/3*np.pi, -np.pi, -2/3*np.pi, -1/3*np.pi, 0, 1/3*np.pi, 2/3*np.pi]
vy = [0.7199,0.7199, 0.7199,0.7199,0.7199,0.7199,0, 0, 0, 0, 0, 0,-0.7199,-0.7199, -0.7199,-0.7199,-0.7199,-0.7199]
pano_mattacher = re.compile(r"[a-zA-Z0-9_]{32}")    
order_mattacher = re.compile(r"[a-zA-Z0-9]+")
...
def fetch_data_file(dataset, index, item):
    ...
    color_name = pano_mattacher.match(item['observation_image']).group(0)
    # Source image
    img_color, img_depth = load_rgbd_as_tensor_pano(
        item['scene_id'], color_name, DS_RATE)
...
def process_input_gpu(source_package, dataset, index, item):
    ...
    '''
    emits = order_mattacher.findall(item['observation_image'])
    # the idx of associated observation angle
    series = int(str(emits[1])[-1])
    n = int(emits[2])
    i = series * 6 + n
    ...
    fx = int((width/2)/np.tan(1.06/2)//DS_RATE)
    fy = int((height/2)/np.tan(1.06/2)//DS_RATE)
    
    cx = int((vx[i]/np.pi+1) * (width/2)//DS_RATE)
    cy = int((vy[i]*2/np.pi+1) * (height/2)//DS_RATE)    
    intrinsics = np.array(
    ...
    tfx = int(configs['intrinsics']['fx'] // DS_RATE)
    tfy = int(configs['intrinsics']['fy'] // DS_RATE)
    tcx = int(configs['intrinsics']['cx'] // DS_RATE)
    tcy = int(configs['intrinsics']['cy'] // DS_RATE)
    ix = int(((float(item['ix'])-tcx)/tfx*fx + cx) // DS_RATE)
    iy = int(((float(item['iy'])-tcy)/tfy*fy + cy) // DS_RATE)
