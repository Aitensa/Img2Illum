...
import vgtk
import EPN.EPNUtils.utils as M
import vgtk.spconv.functional as L


import vgtk.spconv as zptk
import vgtk.so3conv as sptk
class EquFeatExtraction(nn.Module):
    def __init__(self, opt):
        params=build_model(opt)
        super(EquFeatExtraction, self).__init__()

        self.backbone = nn.ModuleList()
        for block_param in params['backbone']:
            self.backbone.append(M.BasicSO3ConvBlock(block_param))

        # self.outblock = M.EquFeatBlockPointnet(params['outblock'])
    ...
    def forward(self, xyz, rgb):
        ...
        # x = torch.cat((xyz,rgb),2)
        # x = M.preprocess_input(x, self.na_in, False)
        x = zptk.SphericalPointCloud(xyz.permute(0,2,1),sptk.get_occupancy_features(rgb,self.na_in,False),None)
...
# Full Version
def build_model(opt,
                mlps=[[32,32], [64,128], [128,128]], # [128,128]],
                out_mlps=[128, 64],
                strides=[2, 2, 2],# 2
                initial_radius_ratio = 0.2,
                sampling_ratio = 0.4, #0.4, 0.36
                sampling_density = 0.7,
                kernel_density = 1,
                kernel_multiplier = 2,
                sigma_ratio= 0.68, # 1e-3, 0.68
    ...
    if input_num > 1280:
        sampling_ratio /= (input_num / 1280)
        strides[0] = int(2 * (input_num / 1280))
        print("Using sampling_ratio:", sampling_ratio)
        print("Using strides:", strides)
